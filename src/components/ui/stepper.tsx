...
function StepperTrigger({ asChild = false, className, children, tabIndex, ...props }: StepperTriggerProps) {
  ...
  return (
    <button
      ...
      className={cn(
        "inline-flex cursor-pointer items-center gap-3 rounded-full outline-none focus-visible:z-10 focus-visible:ring-[3px] disabled:pointer-events-none disabled:opacity-60",
        className,
      )}
...
function StepperIndicator({ children, className }: React.ComponentProps<"div">) {
  ...
  return (
    <div
      ...
      data-state={state}
      className={cn(
        "relative flex size-6 shrink-0 items-center justify-center overflow-hidden rounded-full bg-paper-3 text-xs text-graphite data-[state=active]:bg-ink data-[state=active]:text-paper data-[state=completed]:bg-graphite data-[state=completed]:text-paper",
        className,
      )}
    >
...
function StepperSeparator({ className }: React.ComponentProps<"div">) {
  ...
  return (
    <div
      ...
      data-state={state}
      className={cn(
        "m-0.5 rounded-full bg-line-2 group-data-[orientation=horizontal]/stepper-nav:h-px group-data-[orientation=horizontal]/stepper-nav:flex-1 group-data-[orientation=vertical]/stepper-nav:h-12 group-data-[orientation=vertical]/stepper-nav:w-px",
        className,
      )}
    />
  );
...
function StepperDescription({ children, className }: React.ComponentProps<"div">) {
  ...
  return (
    <div data-slot="stepper-description" data-state={state} className={cn("text-sm text-muted", className)}>
      {children}
    </div>
  );
}
